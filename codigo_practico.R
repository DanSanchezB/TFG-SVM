...
install.packages("caret") #para el train-test y las matrices de confusión con sus métricas
...
#fijamos un train del 70% ya que tampoco tenemos muchísimos individuos


#funciones personalizadas para obtener métricas rendimientos de los modelos ----
evaluar_modelo = function(modelo_svm, datos_test, variable_real, clase_positiva) {
  
  predicciones = predict(modelo_svm, datos_test)
  
  cm = confusionMatrix(data = predicciones, 
                        reference = variable_real, 
                        positive = clase_positiva, 
                        mode = "everything") #así saca todas las métricas
  
  pred_roc = predict(modelo_svm, datos_test, decision.values = TRUE)
  valores_decision = as.numeric(attr(pred_roc, "decision.values"))
  
  curva = roc(response = variable_real, predictor = valores_decision, 
               levels = c("B", "M"), q = TRUE) #q=T quita los avisos
  valor_auc = as.numeric(auc(curva))
  
  tabla = data.frame(
    Metrica = c("Precisión", "Recall (Sensibilidad)", 
                "Especificidad", "Área Bajo la Curva (AUC)"), #son las variables que queremos
    Valor = round(c(
...
evaluar_modelo_multiclase = function(modelo_svm, datos_test, variable_real) {
  ...
  cm = confusionMatrix(data = predicciones, 
                       reference = variable_real, 
                       mode = "everything") # así saca todas las métricas por clase
  ...
  tabla = data.frame(
    Metrica = c("Exactitud (Accuracy)", "Precisión", "Recall (Sensibilidad)", 
                "Especificidad"),
...
breast = read.csv("wdbc.data", header = FALSE) #no tienen títulos las variables
...
colnames(breast) = c("Diagnosis", "m_radius", "m_texture", "m_perimeter", "m_area", 
                      "m_smoothness", "m_compactness", 
                      "m_concavity", "m_concave_points", "m_symmetry", "m_fractal_dimension",
                     ...
                     "w_radius", "w_texture", "w_perimeter", "w_area", 
                     "w_smoothness", "w_compactness", 
...
## breast lineal----
set.seed(28)
param_breast_lineal = tune(svm, Diagnosis ~ ., data = breast_train, kernel = 'linear',
                     scale = TRUE, ranges = list(cost = 10^c(-2:2)) )
...
print(top_5_bl, row.names = T)  #esta tabla está incluida en el anexo del TFG

set.seed(28)
breast_lineal = svm(formula = Diagnosis ~ ., data = breast_train, kernel = "linear", 
    cost = 1, scale = TRUE)
summary(breast_lineal)

pct_sv_breast_lin = (breast_lineal$tot.nSV / nrow(breast_train)) * 100
cat("El SVM Lineal usa el", round(pct_sv_breast_lin, 2), "% de los pacientes del
    entrenamiento como vectores de soporte.\n")
...
## breast polinómico----
set.seed(28)
param_breast_polinomico = tune(svm, Diagnosis ~ ., data = breast_train, kernel = 'polynomial',
                           scale = TRUE, coef0 = 1, ranges = list(cost = 10^c(-2:2), degree = c(2:4)) )
...
print(top_5_bp, row.names = T)  #esta tabla está incluida en el anexo del TFG


set.seed(28)
breast_polinomico = svm(formula = Diagnosis ~ ., data = breast_train, kernel = "polynomial", 
                    cost = 1, degree = 2, coef0 = 1, scale = TRUE)
summary(breast_polinomico)

pct_sv_breast_pol = (breast_polinomico$tot.nSV / nrow(breast_train)) * 100
cat("El SVM polinómico usa el", round(pct_sv_breast_pol, 2), "% de los pacientes
    del entrenamiento como vectores de soporte.\n")
...
## breast radial----
set.seed(28)
param_breast_radial = tune(svm, Diagnosis ~ ., data = breast_train, kernel = 'radial',
                               scale = TRUE, ranges = list(cost = 10^c(-2:2), gamma = 10^c(-4:1)) )
...
print(top_5_br, row.names = T)  #esta tabla está incluida en el anexo del TFG

set.seed(28)
breast_radial = svm(formula = Diagnosis ~ ., data = breast_train, kernel = "radial", 
                        cost = 10, gamma = 0.01, scale = TRUE)
summary(breast_radial)

pct_sv_breast_rad = (breast_radial$tot.nSV / nrow(breast_train)) * 100
cat("El SVM radial usa el", round(pct_sv_breast_rad, 2), "% de los pacientes del
    entrenamiento como vectores de soporte.\n")
...
roc_bl = roc(response = breast_test$Diagnosis, predictor = as.numeric(scores_bl), levels = c("B", "M"))
roc_bp = roc(response = breast_test$Diagnosis, predictor = as.numeric(scores_bp), levels = c("B", "M"))
roc_br = roc(response = breast_test$Diagnosis, predictor = as.numeric(scores_br), levels = c("B", "M"))
...
pdf("roc_breast.pdf", width = 6, height = 6) #para que guarde la imagen

par(family = "serif") #para que ponga una fuente parecida a la de latex


plot(roc_bl, col = "royalblue", lwd = 2, xlab = "1 - Especificidad", 
     ylab = "Sensibilidad", legacy.axes = T, #para que dibuje en el sentido correcto la curva
     cex.lab = 1.4, cex.axis = 1.2) #agranda la letra
plot(roc_bp, col = "red", lwd = 2, add = TRUE)
plot(roc_br, col = "green4", lwd = 2, add = TRUE)

legend("bottomright", 
       legend = c(paste("Kernel Lineal (AUC =", auc_bl, ")"),
...
archivos_dat = list.files(pattern = "\\.dat$")
...
colnames(vehicles) = c("compactness", "circularity", "distance_circularity", "radius_ratio", "pr_axis_aspect_ratio", 
                     "max_length_aspect_ratio", "scatter_ratio", 
                     "elongateness", "pr_axis_rectangularity", "max_length_rectangularity", "scaled_variance_major_axis",
                     "scaled_variance_minor_axis", "scaled_radius_gyration", "skewness_major_axis", "skewness_minor_axis", 
                     "kurtosis_minor_axis", "kurtosis_major_axis", "hollows_ratio", "Vehicle")
...
set.seed(28)
indices_vehicles = createDataPartition(vehicles$Vehicle, p = 0.70, list = FALSE)
...
prop.table(table(vehicles$Vehicle)) # Proporción original
prop.table(table(vehicles_train$Vehicle)) # Proporción en train (debería ser idéntica)
prop.table(table(vehicles_test$Vehicle)) # Proporción en test (debería ser idéntica)


## vehicles lineal----
set.seed(28)
param_vehicles_lineal = tune(svm, Vehicle ~ ., data = vehicles_train, kernel = 'linear',
                           scale = TRUE, ranges = list(cost = 10^c(-2:2)) )
...
cat("\n TOP 5 MODELOS (KERNEL LINEAL): \n")
print(top_5_vl, row.names = T)  #esta tabla está incluida en el anexo del TFG

set.seed(28)
vehicles_lineal = svm(formula = Vehicle ~ ., data = vehicles_train, kernel = "linear", 
                    cost = 10, scale = TRUE)
summary(vehicles_lineal)

pct_sv_vehicles_lin = (vehicles_lineal$tot.nSV / nrow(vehicles_train)) * 100
cat("El SVM Lineal usa el", round(pct_sv_vehicles_lin, 2), "% de los pacientes del entrenamiento como vectores de soporte.\n")
...
## vehicles polinomico----
set.seed(28)
param_vehicles_polinomico = tune(svm, Vehicle ~ ., data = vehicles_train, kernel = 'polynomial',
                             scale = TRUE, coef0 = 1, ranges = list(cost = 10^c(-2:2), degree = c(2:4)) )
...
cat("\n TOP 5 MODELOS (KERNEL POLINÓMICO): \n")
print(top_5_vp, row.names = T)  #esta tabla está incluida en el anexo del TFG

set.seed(28)
vehicles_polinomico = svm(formula = Vehicle ~ ., data = vehicles_train, kernel = "polynomial", 
                      cost = 10, degree = 2, coef0 = 1, scale = TRUE)
summary(vehicles_polinomico)

pct_sv_vehicles_pol = (vehicles_polinomico$tot.nSV / nrow(vehicles_train)) * 100
cat("El SVM Polinómico usa el", round(pct_sv_vehicles_pol, 2), "% de los individuos del entrenamiento como vectores de soporte.\n")
...
## vehicles radial----
set.seed(28)
param_vehicles_radial = tune(svm, Vehicle ~ ., data = vehicles_train, kernel = 'radial',
                             scale = TRUE, ranges = list(cost = 10^c(-2:2), gamma = 10^c(-4:1)) )
...
cat("\n TOP 5 MODELOS (KERNEL RADIAL): \n")
print(top_5_vr, row.names = T)  #esta tabla está incluida en el anexo del TFG

set.seed(28)
vehicles_radial = svm(formula = Vehicle ~ ., data = vehicles_train, kernel = "radial", 
                      cost = 100, gamma = 10^-2, scale = TRUE)
summary(vehicles_radial)

pct_sv_vehicles_rad = (vehicles_radial$tot.nSV / nrow(vehicles_train)) * 100
cat("El SVM radial usa el", round(pct_sv_vehicles_rad, 2), "% de los individuos del entrenamiento como vectores de soporte.\n")
...
grafica_hist_abalone = ggplot(abalone, aes(x = Rings)) +
  ...
  geom_histogram(binwidth = 1, fill = rgb(0.2, 0.5, 0.8, alpha = 0.7), color = "white") +
  labs(
    x = "Número de Anillos",
    y = "Frecuencia"
  ) +
  ...
  theme(
    ...
    panel.grid.major.x = element_blank(), #quitamos la cuadrícula vertical para que quede más limpio
...
ggsave("histograma_abalone.png", plot = grafica_hist_abalone, 
...
## abalone lineal----
set.seed(28)
param_abalone_lineal = tune(svm, Rings ~ ., data = abalone_train, kernel = 'linear',
                             scale = TRUE, ranges = list(cost = 10^c(-1:2), epsilon = c(0.01, 0.05, 0.1, 0.5, 1)) )
...
cat("\n TOP 5 MODELOS (KERNEL LINEAL): \n")
print(top_5_al, row.names = T)  #esta tabla está incluida en el anexo del TFG

set.seed(28)
abalone_lineal = svm(formula = Rings ~ ., data = abalone_train, kernel = "linear", 
                      cost = 10, epsilon = 0.5, scale = TRUE)
summary(abalone_lineal) 

pct_sv_abalone_lin = (abalone_lineal$tot.nSV / nrow(abalone_train)) * 100
cat("La SVR Lineal usa el", round(pct_sv_abalone_lin, 2), "% de los individuos del entrenamiento como vectores de soporte.\n")
...
grafica_al = ggplot(datos_grafica_al, aes(x = Real, y = Predicho)) +
  ...
  geom_point(color = rgb(0.2, 0.5, 0.8), alpha = 0.4, shape = 16, size = 2) +
...
ggsave("abalone_lineal.png", plot = grafica_al, 
       width = 6, height = 4.5, units = "in", dpi = 300, 
       bg = "transparent") #esto último permite que se vea transparente
...
set.seed(28)
param_abalone_polinomico = tune(svm, Rings ~ ., data = abalone_train, kernel = 'polynomial',
                            scale = TRUE, ranges = list(cost = 10^c(-1:2),
                            degree = c(2:4), epsilon = c(0.01, 0.05, 0.1, 0.5, 1)) )
...
cat("\n TOP 5 MODELOS (KERNEL POLINÓMICO): \n")
print(top_5_ap, row.names = T)  #esta tabla está incluida en el anexo del TFG

set.seed(28)
abalone_polinomico = svm(formula = Rings ~ ., data = abalone_train, kernel = "polynomial", 
                     cost = 10, coef0 = 1, degree = 2, epsilon = 0.5, scale = TRUE)
summary(abalone_polinomico) 

pct_sv_abalone_pol = (abalone_polinomico$tot.nSV / nrow(abalone_train)) * 100
cat("La SVR Polinómica usa el", round(pct_sv_abalone_pol, 2), "% de los individuos
    del entrenamiento como vectores de soporte.\n")
...
ggsave("abalone_polinomico.png", plot = grafica_ap, 
       width = 6, height = 4.5, units = "in", dpi = 300, 
       bg = "transparent") #esto último permite que se vea transparente
...
set.seed(28)
param_abalone_radial = tune(svm, Rings ~ ., data = abalone_train, kernel = 'radial',
                                scale = TRUE, ranges = list(cost = 10^c(-1:2), 
                                gamma = 10^c(-3:0), epsilon = c(0.01, 0.05, 0.1, 0.5, 1)) )
...
cat("\n TOP 5 MODELOS (KERNEL RADIAL): \n")
print(top_5_ar, row.names = T)  #esta tabla está incluida en el anexo del TFG

set.seed(28)
abalone_radial = svm(formula = Rings ~ ., data = abalone_train, kernel = "radial", 
                         cost = 10, gamma = 0.1, epsilon = 0.5, scale = TRUE)
summary(abalone_radial) 


pct_sv_abalone_rad = (abalone_radial$tot.nSV / nrow(abalone_train)) * 100
cat("La SVR Radial usa el", round(pct_sv_abalone_rad, 2), "% de los individuos del entrenamiento como vectores de soporte.\n")
...
ggsave("abalone_radial.png", plot = grafica_ar, 
       width = 6, height = 4.5, units = "in", dpi = 300, 
       bg = "transparent") #esto último permite que se vea transparente
